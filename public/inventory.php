...
// require_once __DIR__ . '/../src/Auth.php'; // RBAC removed
require_once __DIR__ . '/../src/Controllers/InventoryController.php';
require_once __DIR__ . '/../src/Helpers/Flash.php';

// Auth::requireStaff('login.php'); // RBAC removed

// $auth = Auth::getInstance(); // RBAC removed
// $user = $auth->currentUser(); // RBAC removed
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['restock_book_id'])) {
    try {
        ...
        $invCtrl->restockBook($bookId, $quantity, $reason, null); // RBAC removed
...
$allBooks = $invCtrl->getAll();
$logs = $invCtrl->getTransactionLogs(null, 20);

// DEBUG: Check what PHP_SELF is
error_log('DEBUG PHP_SELF: ' . $_SERVER['PHP_SELF']);
error_log('DEBUG basename: ' . basename($_SERVER['PHP_SELF']));
                        ...
                        <tbody>
                            <?php foreach ($allBooks as $book): ?>
                                <tr>
                                    ...
                                    <td>
                                        <span class="stock-status-badge status-<?= $book['stock_status'] ?>">
                            ...
                            <tbody>
                                <?php foreach ($logs as $log): ?>
                                    <tr>
                                        ...
                                        <td><strong><?= $log['quantity_change'] ?></strong></td>
                                        <td><?= htmlspecialchars($log['reason'] ?? '-') ?></td>
                                        <td><?= htmlspecialchars($log['restocked_by'] ?? 'System') ?></td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
